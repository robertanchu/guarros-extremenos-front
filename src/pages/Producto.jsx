...
function resolveProduct(slug, location){
  // 1) State from Link
  const st = location?.state?.product || location?.state?.item || null;
  if (st && (st.slug === slug || String(st.id) === String(slug))) return st;

  // 2) Window globals (if you hydrate them on page load)
  const pools = [
    (typeof window !== "undefined" && window.__PRODUCTS__) || [],
    (typeof window !== "undefined" && window.__JAMONES__) || [],
  ];
  for (const arr of pools){
    const found = Array.isArray(arr) && arr.find(p => p?.slug === slug || String(p?.id) === String(slug));
    if (found) return found;
  }

  // 3) LocalStorage (optional)
  if (typeof window !== "undefined"){
    const keys = ["products", "jamones", "catalog"];
    for (const k of keys){
      const data = tryJSON(localStorage.getItem(k));
...
export default function Producto(){
  ...
  const producto = resolveProduct(slug, location);
