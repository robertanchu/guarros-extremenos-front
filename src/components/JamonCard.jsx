...
export default function JamonCard({ product, priceMap = {}, loadingPrices = false }) {
  ...
  // Nota: Dejamos las variables 'unsliced2x' y 'sliced2x' por si las usas en el futuro,
  // pero las quitamos de las etiquetas <img>
  const { unsliced, sliced: slicedImg } = useMemo(() => pickImages(product), [product]);
  ...
  return (
    <div className="group rounded-2xl border border-white/10 bg-white/[0.03] overflow-hidden flex flex-col 
                   ...
                   hover:ring-2 hover:ring-[#E53935]/80"> {/* Efectos hover sin 'md:' */}
      ...
      <div className="relative aspect-[4/3] bg-black/40 overflow-hidden">
        ...
        <img
          ...
          className={`absolute inset-0 w-full h-full object-cover transition-all duration-500 ease-in-out group-hover:scale-[1.05] ${sliced ? "opacity-0" : "opacity-100"}`} // Efecto hover sin 'md:'
          loading="lazy"
          draggable={false}
        />
        {/* SLICED */}
        <img
          ...
          className={`absolute inset-0 w-full h-full object-cover transition-all duration-500 ease-in-out group-hover:scale-[1.05] ${sliced ? "opacity-100" : "opacity-0"}`} // Efecto hover sin 'md:'
          loading="lazy"
          draggable={false}
        />
      </div>
