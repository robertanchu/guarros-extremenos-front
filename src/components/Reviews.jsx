...
function StarRow(){ return (<div className="flex gap-1 text-amber-300">{Array.from({length:5}).map((_,i)=>(<svg key={i} viewBox="0 0 20 20" width="18" height="18" aria-hidden="true"><path fill="currentColor" d="M10 1.5 12.9 7l6.1.9-4.4 4.3 1 6.1L10 15.9 4.4 18.3l1-6.1L1 7.9 7.1 7z"/></svg>))}</div>);}
function Avatar({name}){ const initials = name.split(' ').map(w=>w[0]).join('').slice(0,2).toUpperCase(); return (<div className="w-10 h-10 rounded-full bg-brand/90 text-white flex items-center justify-center text-sm font-semibold ring-2 ring-dorado/50">{initials}</div>); }
        ...
        <div className="max-w-3xl mx-auto">
          <div className="rounded-2xl border border-white/10 bg-white/5 p-6 md:p-8 shadow-[0_10px_40px_rgba(0,0,0,0.35)]">
            <div className="flex items-center gap-4">
              <Avatar name={r.n} />
              <div><div className="text-white font-medium">{r.n}</div><StarRow /></div>
            </div>
          ...
          <div className="flex justify-center gap-2 mt-6">
            {DATA.map((_,idx)=>(<span key={idx} className={"h-2 w-2 rounded-full " + (idx===i ? "bg-amber-300" : "bg-zinc-600")}></span>))}
          </div>
        </div>
      </div>
    </section>
