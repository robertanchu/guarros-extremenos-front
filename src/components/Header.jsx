...
import MobileMenuDrawer from "@/components/MobileMenuDrawer"; import Brand from "./Brand"; import { useCart } from "@/store/cart"; import { useUI } from "@/store/ui";
...
export default function Header(){ const items = useCart(s=>s.items); const total = items.reduce((a,b)=>a+b.qty,0); const cartPulse = useUI(s=>s.cartPulse); const [open,setOpen]=useState(false);
  return (<header className="sticky top-0 z-50 bg-black/70 backdrop-blur border-b border-white/10"><div className="container py-2 md:py-3 flex items-center justify-between">
    ...
    <button onClick={()=> useUI.getState().toggleCart()} className={"relative px-3 py-2 rounded-xl text-zinc-100 hover:bg-white/10 transition " + (cartPulse ? "cart-pulse" : "")} data-cart-target="true" aria-label="Carrito"><CartIcon/><span className="absolute -top-2 -right-2 text-[11px] bg-dorado text-black rounded-full px-2 py-0.5">{total}</span></button>
  </div>
  {/* Mobile menu drawer */}
  {open && (
