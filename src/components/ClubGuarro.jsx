...
export default function ClubGuarro(){
  ...
  useEffect(()=>{ const saved = localStorage.getItem("club_guarro_email"); if(saved){ setEmail(saved); setOk(true);} },[]);
  const submit = (e)=>{ e.preventDefault(); const valid=/.+@.+\..+/.test(email); if(!valid) return alert("Pon un email válido, sin postureo 😉"); localStorage.setItem("club_guarro_email", email); setOk(true); };
  return (
    <section id="club-guarro" className="py-16">
      <div className="shell grid md:grid-cols-2 gap-8 items-center">
        <div>
        ...
        <form onSubmit={submit} className="rounded-2xl border border-white/10 bg-white/5 p-4 md:p-6">
          {!ok ? (
            <div className="flex flex-col sm:flex-row gap-3">
              <input value={email} onChange={e=>setEmail(e.target.value)} type="email" required placeholder="tu@email.com" className="flex-1 bg-black/40 border border-white/10 rounded-xl px-4 py-3 outline-none focus:border-brand" />
