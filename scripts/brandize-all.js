 ...
 * Combined codemod: 
 *  1) red-*   -> brand-*
 *  2) [#HEX]  -> brand(-shade)?  (using config hexToBrand map)
 * Preserves variants (hover:, md:, focus:...), utilities (bg|text|border|ring|from|via|to), and optional opacity (/80).
 */
const fs = require('fs');
...
// B) arbitrary color with HEX: util-[#HEX] or util-[rgb(...)] but we only handle HEX here
// Accept 3/6-digit HEX, case-insensitive
...
function replaceAll(src){
  ...
  // B) arbitrary [#HEX] -> brand(-shade)? (via config map)
  src = src.replace(RE_HEX, (m, ...rest) => {
    ...
    if (!mapped) return m; // leave as-is if not mapped
    changed = true;
    // mapped e.g. 'brand-600' or 'brand-700'
    return `${prefix||''}${util}-${mapped}${opacity||''}`.replace('-brand-600','-brand');
  });

  // C) warn inline styles
  let match;
